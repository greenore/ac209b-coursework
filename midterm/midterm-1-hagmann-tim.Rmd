...
A Dirichlet distribution is the conjugate prior for the multinomial distribution. This is because  posterior Dirichlet distribution p(θ|x) is in the same family as the prior probability distribution p(θ). In other words, because the prior distribution of the multinomial parameters is Dirichlet, the posterior distribution is also a Dirichlet distribution or the  *Dirichlet posterior*  \,\propto\, *Dirichlet prior* x *multinomial likelihood*. 

Concerning the posterior Dirichlet parameters, they equal the sum of the count data and the parameters from the prior Dirichlet, alpha. The distribution can be summarized by the following parameters:
...
(b) From a Monte Carlo simulation of the Dirichlet posterior distribution for "good" restaurants, what is the posterior mean probability that the word "chocolate" is used? 

From a Monte Carlo simulation of the Dirichlet posterior distribution for bad restaurants, what is the posterior mean probability that the word "chocolate" is used?

**Hint**: use the `rdirichlet` function in the `MCMCpack` library. [15 points]
...
# Apply the function
set.seed(123)
post_mean_good <- posterior_mean_R(alpha=2, yA=yA, yB=yB, n.sim=1000)

# Get the probabilty values
df_good <- data.frame(post_mean_good, words)
df_good <- df_good[order(-post_mean_good), ]

prob_good <- df_good[df_good$words == "chocolate", 1]
...
df_pred <- data.frame(df_test$rating, round(prob_dirichlet * 100, 1))
...
```{r}
ggplot() + 
  labs(title="Plot II: Predicted Probabilities", 
       subtitle="of Lennon−composed songs based on melodic transitions") +
...
```{r, include=FALSE}
rm(i, df_pred, y_til, prob_dirichlet, prob_good, prob_bad, yA, yB, post_mean_good)
```
...
The plot above shows, that there is some non-linear behavior present in latidue, longtite and avg_word_count. The RestaurantsPriceRange2 on the other hand appears to be linear. 
...
# Extract values
model_names <- c("Null Model:", "Full w/o splines:", "Optimized w/o splines",
...
# Dataframe
pander(data.frame("Model"=model_names, "Accuracy"=accuracy, "Missclassification"=missclass))
```

The above table shows, that the model with the lowest missclassification rate on the test set is the model without splines and with the insignificant variable **cuisine** excluded. 
...
In order to test if a variable adds value a **likelihood ratio test** can be performed. The test can be used to compare the goodness of fit of two models, one of which (the null model) is a special case of the other (the alternative model). The test is based on the likelihood ratio, which expresses how many times more likely the data are under one model than the other. 
