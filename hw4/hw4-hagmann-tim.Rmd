...
#### Prediction
```{r}
pred_pooled_train <- ifelse(as.numeric(predict(fit_pooled, df_train, type="response")) >= 0.5, "Yes", "No")
pred_pooled_train <- factor(pred_pooled_train, levels=c("No", "Yes"))
pred_pooled_test <- ifelse(as.numeric(predict(fit_pooled, df_test, type="response")) >= 0.5, "Yes", "No")
...
The above table shows that the overall accuracy is at `sprintf("%.1f", conf_mat_test$overall[1] * 100)`%. However, the
...
The above table shows, that for the unpooled model not all coefficients for the different districts are statistically significant. Nevertheless, the model indicates, that the variable *living children* has a different impacts depending on the district.

#### Prediction
```{r, warning=FALSE}
pred_unpooled_train <- ifelse(as.numeric(predict(fit_unpooled, df_train, type="response")) >= 0.5, "Yes", "No")
pred_unpooled_train <- factor(pred_unpooled_train, levels=c("No", "Yes"))
pred_unpooled_test <- ifelse(as.numeric(predict(fit_unpooled, df_test, type="response")) >= 0.5, "Yes", "No")
...
The above table shows that the overall accuracy for the unpooled model is at `sprintf("%.1f", conf_mat_test$overall[1] * 100)`%. That means that the model performs similarly well to the pooled model. However, the model performce better looking at the prediction were it no longer always predicts *No* as the response.
...
fit_hierarchical <- MCMChlogit(fixed=contraceptive_use ~ living_children,
                               ...
                               data=df_train2, r=2, R=diag(c(1, 0.1)), burnin=5000, mcmc=10000,
                               thin=1, verbose=1, beta.start=NA, sigma2.start=NA,
                               Vb.start=NA, FixOD=1, mubeta=c(0,0), Vbeta=10000,
                               nu=0.001, delta=0.001)
```

In the glm case, (a) and (b), we did maximize the likelihood function $p(y|β_0,β_1,x)$. That is we found the weights $β_0, β_1$ that maximize how likely the observed data is. In the case of the bayesian hierarchical logistic regression, the starting point is an initial belief about the distribution of $p(β_0,β_1)$. Then the posterior $p(β_0,β_1|x,y)∝p(y|β_0,β_1,x)p(β_0,β_1)$ which is the updated belief about the weights given evidence, is proportional to our prior (initial belief) times the likelihood. This gives us a distribution over the weights. That means that the bayesian model returns the distributions of the intercept and living children for each district as well as the estimated probability for each seperate district model. The pooled and unpooled model do not do this. 
...
coeff_unpooled <- coeff_unpooled_all[(n_coeff_unpooled - n_districts + 2):n_coeff_unpooled]
...
coeff_hierarchical <- coeff_hierarchical_all[(n_coeff_hierarchical -
                                                5 - n_districts):(n_coeff_hierarchical - 6)]
...
ggplot(df_coeff, aes(districts)) +
  ...
  ggtitle("Estimated coefficient as a function of district") +
  theme_bw() +
  ylim(c(-1.5, 1.5))
```
...
As can be seen in the plot above, the pooled model has by design no variabilty in the coefficient value for living children over the different districts. However, the hierachical bayes as well as the unpooled model vary in there coefficiant value (positive as well as negative) over the different districts. The coefficiants from the hierarchical model appear to be more stable over the different districts compared to the coefficiants from the unpooled model. Furthermore, the number of observations in a district appear to have an effect on the variability of the unpooled as well as the bayesian hierarchical coefficients. The smaller there number the further away from the baseline (pooled). This could indicate some *overfitting* due to the small sample size. 
...
The above plot shows, that the pooled model only has 4 potential values at around 0, 0.3, 0.6 and 1. As was already observed in 2.1 (a) the model is not very good at distinguishing the different classes. The probabilities for the hierarchical bayes as well as the unpooled model are more evenly distributed with 3 clusters at 0.3, 0.5, and 0.6. They are both much better at differentiating the classes at the 0.5 probabilty level.
...
df_mod <- merge(df_mod, data.frame(table(df_train$district)), by.x="district", by.y="Var1")
