...
# write_csv(spam, path="data/lab3_spam.csv")
spam <- data.frame(read_csv("data/lab3_spam.csv"))
str(spam)
```
...
fit_nb <- naiveBayes(spam ~ ., data=spam)
prd_nb <- predict(fit_nb, newdata=spam, type="class")
confmat(spam$spam, prd_nb)
...
```{r}
# Read data into R
# vote <- read.csv("https://archive.ics.uci.edu/ml/machine-learning-databases/voting-records/house-votes-84.data", header=FALSE, na.string="?")
# write_csv(vote, path="data/lab3_vote.csv")
spam <- read_csv("data/lab3_vote.csv")

# Convert everything to numbers
vote[-1] <- lapply(vote[-1], as.numeric)
```
...
```{r}
pa <- class_probability(data=vote[-1], vote[[1]], alpha=1)
```
...
```{r}
confmat(as.numeric(factor(vote[1])), pa > .5)
```
